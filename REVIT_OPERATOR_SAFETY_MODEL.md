...
Private approval-material wrappers support one scoped relaxation for approved
UI-only work:

- `agent-ui-flow-execute-approved-candidate --execute --allow-approved-ui`
- `agent-session-execute-approved-item --execute --allow-approved-ui`

This bypasses the outer `I approve <item-id>` phrase only when the private item
is a UI candidate control or UI workflow step and already contains a fresh
approval token generated by the corresponding dry-run/approval plan. The
underlying action still uses the token-bound safety executor or workflow gate,
captures fresh pre-action observation, stops on modal dialogs, and logs the
receipt as `approval_relaxation_policy:
approved-ui-only-private-material-operator-consent`.

The relaxation never applies to model-changing approval items, model-open
prompts, Save, Sync, Publish, reload-links, detach/upgrade/workset decisions,
destructive dialog buttons, unknown dialogs, or critical add-in bridge
operations.
